...
int main(){
	...
	head = NULL;
	insertfirst(1,"Ham Burger",23,120.23);
	insertend(2,"Cheese Burger",13,100.67);
		...
		if(main_menu_choice == 1){
			...
			if(admin_panel_choice==1234567){
				...
				if(adminchoise==1){
					...
					br(3);pre(4);      printf(" Enter Your Burger Name :  ");
					...
					scanf("%d",&fquantity); fflush(stdin);
                        ...
                        foodserial:
					br(2);pre(4);  printf(" Enter Burger ID :  ");
                      scanf("%d",&fdata);
                            node *exist;
                            exist = list;
                      ...
                      if(exist->data==fdata){
                       cls(); br(5);pre(3);  printf(" Items Already Exist, Please Re-Enter  "); Sleep(2000);
                       goto foodserial;
                      }
	...
	if(get_food_choice == temp->data){
		...
		if(confirm == 1 ){
			...
			if(payment==1){
				...
				br(2);pre(4);  printf("1. Want Another? ");
...
void main_menu(){
	...
	cls();
	br(5); pre(3); echo(">> 1. burger Lists"); Sleep(400);
...
void pwellcome(){
	...
	char welcome2[50]=" TO";
	char welcome3[50]=" BIG MAC BURGER HOUSE";
